...
export default function Home() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setLoading(false);
    }, 4500); // Increased duration to let the full animation play

    return () => clearTimeout(timer);
  }, []);

  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-black via-slate-900 to-background">
       <AnimatePresence>
        {loading && (
            <motion.div
                key="preloader"
